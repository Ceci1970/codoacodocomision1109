...
public class AlumnosDAO {
    ...
    public Alumnos mostrarAlumno(int _id){
        ...
        try{
            ps = conexion.prepareStatement("SELECT id, nombres,apellidos,email,telefono FROM participante Where id = ?");
            ps.setInt(1, _id);
            rs = ps.executeQuery();
    ...
    public boolean insertarAlumno(Alumnos alumno){
        PreparedStatement ps;
        try{
          ps = conexion.prepareStatement("INSERT INTO participante(nombres,apellidos,email, telefono)VALUES(?,?,?,?)");
    ...
    public boolean actualizarAlumno(Alumnos alumno){
        PreparedStatement ps;
         ...
         try{
          ps = conexion.prepareStatement("UPDATE participante SET nombres=?, apellidos=?, email=?,telefono=? WHERE id=?");
          ps.setString(1,alumno.getNombres());
          ps.setString(2,alumno.getApellidos());
          ps.setString(3,alumno.getEmail());
          ps.setString(4,alumno.getTelefono());
